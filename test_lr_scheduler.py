...
class MyTestCase(unittest.TestCase):
    ...
    def test_reduce_lr_on_plateau1(self):
        ...
        scheduler = ReduceLROnPlateau(self.opt, threshold_mode='abs', mode='min', threshold=0.1, patience=5, cooldown=5)
        epochs = 10
    ...
    def test_reduce_lr_on_plateau5(self):
        ...
        epochs = 20
        self._test_reduce_lr_on_plateau(scheduler, targets, metrics, epochs, 0)
        ...
        for epoch in range(epochs):
            ...
            for param_group, target in zip(self.opt.param_groups, targets):
                self.assertAlmostEquals(target[epoch], param_group['lr'],
                                        msg='LR is wrong in epoch {}: expected {}, got {}'.format(epoch, target[epoch],
                                                                                                  param_group['lr']),
                                        delta=1e-5)
